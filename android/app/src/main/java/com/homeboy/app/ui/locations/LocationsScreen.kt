...
fun LocationsTab() {
    ...
    LaunchedEffect(Unit) {
        if (tree.isNotEmpty()) expandedNodeIds = tree.map { it.id }.toSet()
    }

    LaunchedEffect(tree) {
        if (tree.isNotEmpty() && expandedNodeIds.isEmpty()) {
            expandedNodeIds = tree.map { it.id }.toSet()
        }
    }
            ...
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { isGridMode = !isGridMode }) {
                        Icon(
                            if (isGridMode) Icons.Default.ViewList else Icons.Default.AccountTree,
                            contentDescription = if (isGridMode) "List view" else "Org chart view"
                        )
                    }
                },
            ...
            Column(
                ...
                tree.forEach { node ->
                    OrgChartNode(
                        ...
                        onToggleExpand = { id ->
                            expandedNodeIds = if (id in expandedNodeIds)
                                expandedNodeIds - id else expandedNodeIds + id
                        },
...
private fun OrgChartNode(
    ...
    Column(Modifier.fillMaxWidth()) {
        Row(
            ...
            if (depth > 0) {
                // Horizontal connector line from parent vertical line to card
        ...
        AnimatedVisibility(
            ...
            Box(Modifier.fillMaxWidth()) {
                // Vertical connecting line from this node down through all children
                Canvas(
                    Modifier
                        .fillMaxWidth()
                        .matchParentSize()
